...
describe('close', function () {
  ...
  describe('execute', function () {
    ...
    it('should be succesful when door is open', function () {
      ...
      expect(socket.broadcast.to().emit).toHaveBeenCalledWith('output', { message: 'TestUser closes the door to the south.' });
    ...
    it('should be succesful when door is already closed', function () {
      ...
      expect(exit.closed).toBe(true);
      expect(socket.broadcast.to().emit).toHaveBeenCalledWith('output', { message: 'TestUser closes the door to the north.' });
      expect(socket.emit).toHaveBeenCalledWith('output', { message: 'Door closed.' });
    });
