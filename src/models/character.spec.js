...
describe('character model', () => {
  ...
  describe('update', () => {
    // mockingoose.Quest.toReturn(undefined, 'update');
    let mockQuestOne;
    let mockQuestTwo;

    beforeEach(() => {
      mockQuestOne = createMockQuest();
      mockQuestTwo = createMockQuest();

      character = new Character();
      character.quests = [mockQuestOne, mockQuestTwo];
      character.states = [Object.create({
        update: jest.fn(),
      })],
      character.save = jest.fn();
    });
    ...
    test('calls quest.update for each quest', () => {
      character.update();

      expect(mockQuestOne.update).toHaveBeenCalledWith(character);
      expect(mockQuestTwo.update).toHaveBeenCalledWith(character);
    });
  });

  function createMockQuest() {
    let quest = new Quest();
    quest.update = jest.fn();
    quest.save = jest.fn();
